...
class Connector(threading.Thread):
    ...
    def oplog_thread_join(self):
        ...
        if s:
            s.starttls()
            s.login("systemalerts@lifcare.in", "lifcare@12")

            # msg = MIMEMultipart()
            # msg['From']="systemalerts@lifcare.in"
            # msg['To']="devratna.arya@lifcare.in"
            # msg['Subject']="MongoConnector Status"
            now = datetime.datetime.now()
            message = "MongoConnector stopped!! Please look into it. ("+str(now)+")"

            # msg.attach(MIMEText(message, 'plain'))
            s.sendmail("systemalerts@lifcare.in", "devratna.arya@lifcare.in", message)
